...
export class RoundListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.baseStatuses=[
      {value:1,viewValueEn:'Pending Approval',viewValueAr:'بإنتظار الموافقة'},
  ...
  public getFormData() {
    this.baseService.getItemsByKey(Controllers.User, Actions.GetAllItems, 'role', this.userRolesEnum.Merchant).subscribe(res => {
      this.clients = res;
      this.dynamicService.getFormSettings('FinancialRoundSearchForm').subscribe(res => {
  ...
  public getListSettings() {
    this.dynamicService.getListSettings('FinancialRoundList').subscribe(res => {
  ...
  public getListData(pageSize?: number, pageNumber?: number) {
    ...
    this.baseService.postItem(Controllers.FinancialRound, Actions.GetList, request).subscribe(res => {
  ...
  public serveAction(event: DynamicFormOutput) {
    ...
    this.baseService.postItem(Controllers.FinancialRound, Actions.GetList, event.data).subscribe(res => {
      this.roundList = res.entities;
      this.dynamicListInput.data = this.roundList;
      this.dynamicListInput.totalCount = res.totalCount;
      this.spinner.hide();
